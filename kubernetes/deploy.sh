...
# roles
gen_template "rbac.yaml" | kubectl  -n ${namespace} ${ACTION} -f -

# create the secrets
gen_template "secrets.yaml" | kubectl  -n ${namespace} ${ACTION} -f -

# create the storage pv
gen_template "pv.yaml" | kubectl  -n ${namespace} ${ACTION} -f -

# create pvcs
gen_template "pvc.yaml" | kubectl  -n ${namespace} ${ACTION} -f -

# create the hub
# this doesn't work for some reason
# gen_template "configmap.yaml" | kubectl -n ${namespace} ${ACTION} -f -
...
gen_template "jupyterhub.yaml" | kubectl -n ${namespace} ${ACTION} -f -
